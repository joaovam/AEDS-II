...
int achaPosicaoMenor(PJogador *array,int menor,int n,int j,int *comp){
	...
	if (j < n)
        {	
        	*comp  = *comp + 1;
            ...
            menor = achaPosicaoMenor(array,menor, n,  ++j,comp);
        }
...
void selecao(PJogador *array, int n, int i, int j,int *comp)
{
    if (i < (n - 1))
    {
        int menor = i;
        ...
        menor = achaPosicaoMenor(array,menor,n,j,comp);
        swapJogador(array[menor], array[i]);
        i++;
        selecao(array, n, i, i + 1,comp);
...
void criaLog(double tempo, int comp)
{
    ...
    FILE *fp = fopen("698159_selecaoRecursiva.txt", "w");
