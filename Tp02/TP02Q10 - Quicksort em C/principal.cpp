...
void quicksort(PJogador *array, int esq, int dir, int *comp)
{
    int i = esq, j = dir;
    PJogador pivo = (PJogador)malloc(sizeof(NJogador));
    ...
    free(pivo);
    if (esq < j)
        quicksort(array, esq, j, comp);
    if (i < dir)
        quicksort(array, i, dir, comp);
}

void criaLog(double tempo, int comp)
{

    FILE *fp = fopen("698159_quicksort.txt", "w");
